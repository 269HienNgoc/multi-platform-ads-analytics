...
Configure the Graph API version explicitly with `ADS_META_VERSION`; do not assume the repository default remains valid indefinitely. Tokens are server-only and must never be returned by an API response or exposed through `NEXT_PUBLIC_*` variables.
